...
function randomImage() {
   ...
   var allImages = getElement('employeeDirectorySection').querySelectorAll('img');
...
function cancelUpdate() : void {
    getFormElement('addEmployeeForm').reset();
    getFormElement('entityForm').reset();
...
function getCountOfEmployee() : void {

    for (let i = 0; i < entityObject.departmentList.length; i++) {
        departmentCount[i] = usersDirectory.filter(user => user.department.toLowerCase() === departments[i].toLowerCase()).length;
    }

    for (let i = 0; i < entityObject.officeList.length; i++) {
        officesCount[i] = usersDirectory.filter(user => user.office.toLowerCase() === offices[i].toLowerCase()).length;
    }

    for (let i = 0; i < entityObject.jobTitleList.length; i++) {
        jobTitlesCount[i] = usersDirectory.filter(user => user.jobTitle.toLowerCase() === jobTitles[i].toLowerCase()).length;
...
function displayDepartmentCount() {
    ...
    for (let i = 0; i < entityObject.departmentList.length; i++) {
        department[i].innerHTML = String(departmentCount[i]);
    }
    
    let offices = getByClassName('office');
    for (let i = 0; i < entityObject.officeList.length; i++) {
        offices[i].innerHTML = String(officesCount[i]);
    }

    let jobTitles = getByClassName('jobTitle');
    for (let i = 0; i < entityObject.jobTitleList.length; i++) {
        jobTitles[i].innerHTML = String(jobTitlesCount[i]);
...
function clearSearch() {
    getInputElement('searchEmployee').value = '';
    clearEmployee();
...
function toogleEditButton(isValid) {
    ...
    isValid ? editButton.removeAttribute('data-dismiss') : editButton.setAttribute('data-dismiss', 'modal');
...
function filterBySelect(selected) {
   ...
   var filterBy = selected.value; // value of selected may be department, office or jobTitle or firstName
...
var selectOption = getSelectElement('selectEntity');
...
function addFilter() {
    ...
    var entity = getSelectElement('selectEntity').value;
    var entityType = getSelectElement('entityType').value;
